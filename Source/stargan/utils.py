...
def load_test_data(image_path, size=128):
    # migrated from misc: img = misc.imread(image_path, mode='RGB')
    img = imageio.imread(image_path, pilmode='RGB')

    # migrated from misc: img = misc.imresize(img, [size, size])
...
def save_images(images, size, image_path):
    return imsave(convert_to_imageio_expected_range(images).astype('uint8'),
                  size, image_path)
...
def merge(images, size):
    ...
    if images.shape[3] in (3, 4):
        c = images.shape[3]
        img = np.zeros((h * size[0], w * size[1], c), 'uint8')
        ...
        img = np.zeros((h * size[0], w * size[1]), 'uint8')
        for idx, image in enumerate(images):
            i = idx % size[1]
            j = idx // size[1]
...
# float64 [-1, 1] -> uint8 [0, 255]
def convert_to_imageio_expected_range(x: np.ndarray) -> np.ndarray:
    return (x + 1.) * 127.5
