...
class StarGAN(object):
    def __init__(self, sess, args):
        ...
        self.export_dir_base = args.export_dir_base
        self.augment_flag = args.augment_flag
        self.model_ver = args.model_ver
    ...
    def build_model(self):
        ...
        """ Input Image"""
        Image_data_class = ImageData(load_size=self.img_size, channels=self.img_ch, data_path=self.dataset_path,
        ...
        train_dataset = train_dataset. \
            ...
            apply(map_and_batch(Image_data_class.image_processing, self.batch_size, num_parallel_batches=8,
                                drop_remainder=True)). \
        ...
        self.custom_label_fix_list = tf.placeholder(dtype=tf.float32,
                                                    shape=(self.c_dim, 1, self.c_dim),  # default shape=(5, 1, 5)
                                                    name='custom_label_fix_list')
        ...
        """ Result Image """
        self.x_fake_list = tf.map_fn(lambda x: self.generator(self.x_real, x, reuse=True), label_fix_list,
                                     dtype=tf.float32)

        """ Test Image """
        self.x_test_fake_list = tf.map_fn(lambda x: self.generator(self.x_test, x, reuse=True), test_label_fix_list,
                                          dtype=tf.float32)
        self.custom_fake_image = tf.map_fn(lambda x: self.generator(self.custom_image, x, reuse=True),
                                           self.custom_label_fix_list, dtype=tf.float32)
        ...
        self.g_optimizer = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(self.g_loss,
                                                                                            var_list=G_vars)
        self.d_optimizer = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(self.d_loss,
                                                                                            var_list=D_vars)
    ...
    def test(self):
        ...
        for sample_file in test_files:
            ...
            labels = create_labels(self.custom_label, self.selected_attrs)
            labels = np.transpose(labels, axes=[1, 0, 2])

            fake_image = self.sess.run(self.custom_fake_image,
                                       feed_dict={
                                           self.custom_image: sample_image,
                                           self.custom_label_fix_list: labels
                                       })
    ...
    def export(self):
        ...
        # default path: "./saved_model/0"
        export_path = os.path.join(self.export_dir_base, self.model_ver)

        input_img = self.custom_image
        labels = self.custom_label_fix_list
        output_img = self.custom_fake_image

        tf.saved_model.simple_save(self.sess,
                                   export_path,
                                   inputs={
                                       'custom_image': input_img,
                                       'custom_label_fix_list': labels
                                   },
                                   outputs={
